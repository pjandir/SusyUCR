...
#include "histio.c"
  ...
  //--------------------------------------------------------------------------------------------------------
   ...
   void draw_bgsig_hists4( const char* infile_bg  = "outputfiles/fill-bg-hists4.root",
                          const char* infile_sig = "outputfiles/fill-sig-hists4.root",
  ...
  //====================================================================================================================
   ...
   void draw_stack( const char* configstr, int nbi, int nji ) {
      ...
      for ( int si=0; si<n_samples; si++ ) {
         ...
         if ( !issignal[si] ) {
            if ( si == 0 ) {
               ...
               sprintf( htitle, "Njet%d %s", nji, configstr ) ;
               h_bgsum -> SetTitle( htitle ) ;
               sprintf( hname, "h_bgsigsum_%s_nj%d", configstr, nji ) ;
               h_bgsigsum = (TH1F*) h1d -> Clone( hname ) ;
               h_bgsigsum -> SetTitle( htitle ) ;
            } else {
               h_bgsum -> Add( h1d ) ;
               h_bgsigsum -> Add( h1d ) ;
            }
      ...
      for ( int si=0; si<n_samples; si++ ) {
         ...
         char htitle[1000] ;
         sprintf( htitle, "Njet%d %s fraction", nji, h1d_samples[si] -> GetTitle() ) ;
      ...
      gPad -> SetLogy(0) ;
      sprintf( fname, "outputfiles/bgplot-nj%d-%s-%s-liny.pdf", nji, configstr, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;

      gPad -> SetLogy(1) ;
      sprintf( fname, "outputfiles/bgplot-nj%d-%s-%s-logy.pdf", nji, configstr, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
      ...
      gPad -> SetLogy(0) ;
      sprintf( fname, "outputfiles/bgplot-nj%d-%s-%s-zoom.pdf", nji, configstr, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
      ...
      gPad -> SetLogy(0) ;
      sprintf( fname, "outputfiles/bgplot-nj%d-%s-%s-zoom2.pdf", nji, configstr, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
      ...
      sprintf( fname, "outputfiles/bgplot-nj%d-%s-%s-frac.pdf", nji, configstr, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
