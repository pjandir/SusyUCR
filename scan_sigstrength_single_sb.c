...
#include <sstream>
  ...
  using namespace RooStats;
   ...
   void scan_sigstrength_single_sb( const char* wsfile = "outputfiles/ws-t1bbbbH.root",
                          const char* sb_name = "SB-Njet9+-Nb3+-MHT3-HT23",
                          const char* outfilebase = "signal-strength-scan-t1bbbbH",
                          double scanLow = 0., double scanHigh = 3.0, int nScanPoints = 10, double ymax = 9.  ) {
      ...
      sprintf( pname, "sb_index_%s", sb_name ) ;
      const RooConstVar* rv_sb_index = (const RooConstVar*) ws->obj( pname ) ;
      if ( rv_sb_index == 0x0 ) { printf("\n\n *** %s missing from workspace.\n\n", pname ) ; return ; }
      ...
      printf("  Finding PDFs in likelihood with %s = %.0f\n", rv_sb_index -> GetName(), rv_sb_index -> getVal() ) ;
      ...
      while ( RooAbsPdf* pdf = (RooAbsPdf*) pdf_iter.Next() ) {
         ...
         if ( rv_pdf_sb_index->getVal() == rv_sb_index -> getVal() || rv_pdf_sb_index->getVal() < 0 ) {
            pdf_subset.add( *pdf ) ;
            printf(" *\n") ;
         } else {
            printf("\n") ;
      ...
      RooProdPdf* new_likelihood = new RooProdPdf( "new_likelihood", "new_likelihood", pdf_subset ) ;
      ...
      for ( int spi=0; spi<nScanPoints; spi++ ) {
         ...
         //// RooFitResult* fitResult_sp = new_likelihood -> fitTo( *rds, Save(true), Hesse(false), Strategy(1), PrintLevel(0) ) ;
