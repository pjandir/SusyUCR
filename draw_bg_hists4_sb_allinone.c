...
#include <fstream>
  ...
  //--------------------------------------------------------------------------------------------------------
   ...
   void draw_bg_hists4_sb_allinone( const char* infile = "outputfiles/fill-bg-hists4.root",
                                    const char* lhbuilder_file = "outputfiles/lhbuilder-input-v4b-perfect-closure-ss1-fullfit-t1bbbbH.txt"
                                  ) {
      ...
      can = (TCanvas*) gDirectory -> FindObject( "can_draw_bg_hists4_sb_allinone" ) ;
      if ( can == 0x0 ) can = new TCanvas( "can_draw_bg_hists4_sb_allinone", "BG hists", 1500, 750 ) ;
      can -> Clear() ;
  ...
  //====================================================================================================================
   ...
   void draw_stack( const char* selname ) {
      ...
      for ( int si=0; si<n_samples; si++ ) {
         ...
         TH1F* h1p = new TH1F( hname1d, htitle1d, n_search_bins, 0.5, n_search_bins+0.5 ) ;
         ...
         for ( int sbi=0; sbi<n_search_bins; sbi++ ) {
            ...
            char binlabel[1000] ;
            sprintf( binlabel, "%s %2d", sb_name[sbi], sbi+1 ) ;
            h1p -> GetXaxis() -> SetBinLabel( sbi+1, binlabel ) ;

            h1p -> SetBinContent( sbi+1, val ) ;
            h1p -> SetBinError( sbi+1, sqrt(err2) ) ;
