...
#include <fstream>
  ...
  //--------------------------------------------------------------------------------------------------------
   ...
   void draw_bgsig_hists4_allinone( const char* bg_infile = "outputfiles/fill-bg-hists-tm4.root",
                                    const char* sig_infile = "outputfiles/fill-sig-hists-tm4.root",
      ...
      gDirectory -> Delete( "h*" ) ;
      printf("\n\n Reading in BG 3D histograms from %s\n\n", bg_infile ) ;
      loadHist( bg_infile ) ;
      printf("\n\n Reading in SIG 3D histograms from %s\n\n", sig_infile ) ;
      loadHist( sig_infile ) ;
      ...
      can = (TCanvas*) gDirectory -> FindObject( "can_draw_bgsig_hists4_allinone" ) ;
      if ( can == 0x0 ) can = new TCanvas( "can_draw_bgsig_hists4_allinone", "BG hists", 1500, 750 ) ;
      can -> Clear() ;
      ...
      TString savefilets( sig_infile ) ;
      char newend[100] ;
      sprintf( newend, "-allinone-%s-postdraw.root", signame ) ;
      savefilets.ReplaceAll( ".root", newend ) ;
      printf("\n\n Saving all histograms as %s\n", savefilets.Data() ) ;
      saveHist( savefilets.Data(), "h*" ) ;


   } // draw_bgsig_hists4_allinone
  ...
  //====================================================================================================================
   ...
   void draw_stack( const char* selname ) {
      ...
      for ( int si=0; si<n_samples; si++ ) {
         ...
         if ( !issignal[si] ) {
            if ( si == 0 ) {
               ...
               sprintf( hname1d, "h_1d_allinone_%s_bgsigsum", selname ) ;
               h_bgsigsum = (TH1F*) h1p -> Clone( hname1d ) ;
               sprintf( htitle1d, "%s, bgsigsum", selname ) ;
      ...
      gPad -> SetLogy(0) ;
      sprintf( fname, "outputfiles/bgplot-1d-allinone-%s-%s-liny.pdf", selname, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;

      gPad -> SetLogy(1) ;
      sprintf( fname, "outputfiles/bgplot-1d-allinone-%s-%s-logy.pdf", selname, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
      ...
      gPad -> SetLogy(0) ;
      sprintf( fname, "outputfiles/bgplot-1d-allinone-%s-%s-zoom.pdf", selname, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
      ...
      gPad -> SetLogy(0) ;
      sprintf( fname, "outputfiles/bgplot-1d-allinone-%s-%s-zoom2.pdf", selname, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
      ...
      sprintf( fname, "outputfiles/bgplot-1d-allinone-%s-%s-frac.pdf", selname, signal_name ) ;
      can -> Update() ; can -> Draw() ;
      can -> SaveAs( fname ) ;
