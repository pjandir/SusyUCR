...
#include <fstream>
  ...
  //=================================================================================
   ...
   void build_ra2b_ws4b( const char* infile = "outputfiles/lhbuilder-input-v4b-t1bbbbH.txt",
      ...
      float fb_rslldp_ldp_val[MAX_FB] ;
      float fb_rslldp_ldp_err[MAX_FB] ;
      float fb_nzl[MAX_FB] ;
      float fb_nsl[MAX_FB] ;
      float fb_nldp[MAX_FB] ;
      float fb_nslldp[MAX_FB] ;
      float fb_sig_nzl[MAX_FB] ;
      float fb_sig_nsl[MAX_FB] ;
      float fb_sig_nldp[MAX_FB] ;
      float fb_sig_nslldp[MAX_FB] ;
      ...
      for ( int fbi=0; fbi<n_fine_bins; fbi++ ) {
         ...
         sscanf( line.Data(), "%s %f %f  %f %f  %f %f %f %f  %f %f %f %f",
           fb_name[fbi],
           &(fb_rsl_zl_val[fbi]),
           &(fb_rsl_zl_err[fbi]),
           &(fb_rslldp_ldp_val[fbi]),
           &(fb_rslldp_ldp_err[fbi]),
           &(fb_nzl[fbi]),
           &(fb_nsl[fbi]),
           &(fb_nldp[fbi]),
           &(fb_nslldp[fbi]),
      ...
      for ( int fbi=0; fbi<n_fine_bins; fbi++ ) {
         printf( " %3d : %20s  %7.1f %7.1f %7.1f %7.1f\n", fbi, fb_name[fbi], fb_nzl[fbi], fb_nsl[fbi], fb_nldp[fbi], fb_nslldp[fbi] ) ;
      } // fbi.
      ...
      for ( int sbi=0; sbi<n_search_bins; sbi++ ) {
         ...
         for ( int i=0; i<sb_nfb[sbi]; i++ ) {
            ...
            printf( "      %s : observables : Nldp = %10.3f, Nsl = %10.3f, Nzl = %10.3f, Nslldp = %10.3f\n", 
                          fb_name[fbi], fb_nldp[fbi], fb_nsl[fbi], fb_nzl[fbi], fb_nslldp[fbi] ) ;
            ...
            printf("\n --- Single Lepton, Low min Delta Phi:\n") ;

            sprintf( pname, "mu_ll_slldp_%s", fb_name[fbi] ) ;
            RooRealVar* rv_mu_ll_slldp = new RooRealVar( pname, pname, fb_nslldp[fbi], 0., get_par_max( fb_nslldp[fbi] ) ) ;
            rv_mu_ll_slldp -> setConstant( kFALSE ) ;
            rv_mu_ll_slldp -> Print() ;
            allBGmuPars -> add( *rv_mu_ll_slldp ) ;

            sprintf( pname, "mu_sig0_slldp_%s", fb_name[fbi] ) ;
            RooRealVar* rv_mu_sig0_slldp = new RooRealVar( pname, pname, fb_sig_nslldp[fbi], 0., 1.e6 ) ;
            rv_mu_sig0_slldp -> setConstant( kTRUE ) ;
            rv_mu_sig0_slldp -> Print() ;

            sprintf( pname, "mu_sig_slldp_%s", fb_name[fbi] ) ;
            RooFormulaVar* rv_mu_sig_slldp = new RooFormulaVar( pname, "@0 * @1", RooArgSet( *rv_sig_strength, *rv_mu_sig0_slldp ) ) ;
            rv_mu_sig_slldp -> Print() ;

            sprintf( pname, "n_slldp_%s", fb_name[fbi] ) ;
            RooFormulaVar* rv_n_slldp = new RooFormulaVar( pname, "@0 + @1", RooArgSet( *rv_mu_ll_slldp, *rv_mu_sig_slldp ) ) ;
            rv_n_slldp -> Print() ;

            sprintf( pname, "Nslldp_%s", fb_name[fbi] ) ;
            RooRealVar* rv_Nslldp = new RooRealVar( pname, pname, 0., 1.e6 ) ;
            rv_Nslldp -> setVal( fb_nslldp[fbi] ) ;
            rv_Nslldp -> setConstant( kTRUE ) ;
            rv_Nslldp -> Print() ;
            observedParametersList -> add( *rv_Nslldp ) ;

            sprintf( pname, "pdf_slldp_%s", fb_name[fbi] ) ;
            RooPoissonLogEval* rv_pdf_slldp = new RooPoissonLogEval( pname, pname, *rv_Nslldp, *rv_n_slldp, no_rounding ) ;
            rv_pdf_slldp -> Print() ;

            pdflist.add( *rv_pdf_slldp ) ;

            sprintf( pname2, "%s_sb_index", pname ) ;
            RooConstVar* rv_pdf_sb_index_slldp = new RooConstVar( pname2, pname2, sbi ) ;
            pdf_sbIndexList -> add( *rv_pdf_sb_index_slldp ) ;
            ...
            float rslldp_ldp_val = fb_rslldp_ldp_val[fbi] ;
            float rslldp_ldp_err = fb_rslldp_ldp_err[fbi] ;
            if ( rslldp_ldp_val <= 0. ) {
               rslldp_ldp_val = 1.0 ;
               rslldp_ldp_err = 1.0 ;
            } else if ( rslldp_ldp_err > 3 ) {
               rslldp_ldp_val = 2.0 ;
               rslldp_ldp_err = 2.0 ;
            }

            sprintf( pname, "R_slldp_ldp_%s", fb_name[fbi] ) ;
            RooAbsReal* rv_R_slldp_ldp = makeLognormalConstraint( pname, rslldp_ldp_val, rslldp_ldp_err, sbi ) ;
            rv_R_slldp_ldp -> Print() ;

            sprintf( pname, "mu_ll_ldp_%s", fb_name[fbi] ) ;
            RooFormulaVar* rv_mu_ll_ldp = new RooFormulaVar( pname, "@0 * @1", RooArgSet( *rv_R_slldp_ldp, *rv_mu_ll_slldp ) ) ;
            rv_mu_ll_ldp -> Print() ;
